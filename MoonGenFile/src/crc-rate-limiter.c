...
void moongen_send_all_packets_with_first_delay_bad_crc(uint8_t port_id, uint16_t queue_id, struct rte_mbuf** load_pkts, uint16_t num_pkts, struct rte_mempool* pool, uint32_t min_pkt_size, uint32_t byteDelay){
  const int BUF_SIZE = 128;
  struct rte_mbuf* pkts[BUF_SIZE];
  int send_buf_idx = 0;
  uint32_t num_bad_pkts = 0;
  ...
  dpdk_send_all_packets(port_id, queue_id, load_pkts, num_pkts);

  
  // step 1: generate delay-packets
  while ( byteDelay > 0) {
    struct rte_mbuf* pkt = get_delay_pkt_bad_crc(pool, &byteDelay, min_pkt_size);
    if (pkt) {
      num_bad_pkts++;
      // packet size: [MAC, CRC] to be consistent with HW counters
      num_bad_bytes += pkt->pkt_len;
