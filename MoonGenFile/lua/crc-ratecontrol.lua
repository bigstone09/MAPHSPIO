...
ffi.cdef[[
	...
	void moongen_send_all_packets_with_first_delay_bad_crc(uint8_t port_id, uint16_t queue_id, struct rte_mbuf** load_pkts, uint16_t num_pkts, struct mempool* pool, uint32_t min_pkt_size, uint32_t byteDelay);
]]
...
--   increases precision at low non-cbr rates
-- @param n optional, number of packets to send (defaults to full bufs)
function txQueue:sendBatchWithDelay(bufs, targetRate, n, byteDelay )
        if not self.dev.crcPatch then
                log:fatal("Driver does not support disabling the CRC flag. This feature requires a patched driver.")
        end
        targetRate = targetRate or 14.88
        ...
        n = n or bufs.size
        --local avgPacketSize = 1.25/targetRate * 1000
        local minPktSize = self.dev.minPacketSize or 64
        local maxPktRate = self.dev.maxPacketRate or 14.88
		--	print("maxPktRate:"..maxPktRate)
		--	print("minPktSize:"..minPktSize)
        -- allow smaller packets at low rates
        if targetRate < maxPktRate / 2 then
                minPktSize = minPktSize + 20
        else
        ...
        C.moongen_send_all_packets_with_first_delay_bad_crc(self.id, self.qid, bufs.array, n, mempool, minPktSize, byteDelay)
        return n
end
